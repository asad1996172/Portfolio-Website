...
def main():
    ...
    df['colored_tags'] = df.apply(
        lambda x: generate_colored_tags([elem.strip() for elem in x['Tags'].split('||')], colored_tag_dict), axis=1)

    unique_tags = generate_colored_tags(unique_tags, colored_tag_dict)

    rendered_page = render_template('index.html', about_data=about_section_data,
                                    publications_data=publications_section_data,
    ...
    rendered_page = rendered_page.replace('/static/', 'static/')
